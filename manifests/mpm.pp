define apache::mpm (
  ...
  case $::osfamily {
    'debian': {
      ...
      if $mpm == 'itk' and $::operatingsystem == 'Ubuntu' and $::operatingsystemrelease == '18.04' {
        # workaround https://bugs.launchpad.net/ubuntu/+source/mpm-itk/+bug/1286882
        exec {
          '/usr/sbin/a2dismod mpm_event':
            onlyif  => '/usr/bin/test -e /etc/apache2/mods-enabled/mpm_event.load',
            require => Package['httpd'],
            before  => Package['libapache2-mpm-itk'],
        }
      }

      if $mpm == 'itk' and ( ( $::operatingsystem == 'Ubuntu' and ( $::operatingsystemrelease == '16.04' or $::operatingsystemrelease == '18.04' ) ) or ( $::operatingsystem == 'Debian' and versioncmp($::operatingsystemrelease, '9.0.0') >= 0 ) ) {
